...
sys.path.append('../')
import project.ui__select.ui_select as ui
...
class myMainWindow(QMainWindow, ui.Ui_MainWindow):
    def __init__(self):
        try:
            ...
            """
            self.robot1_assessment_button.clicked.connect(self.robot1_assessment_button_click)
            self.robot2_assessment_button.clicked.connect(self.robot2_assessment_button_click)
    ...
    #
    def robot1_assessment_button_click(self):
        try:
            self.robot1_assessment_button.setStyleSheet("background-color: green")
            oa_zones = 5
            oa_hits = 5
            _msg = configs.MessageHelpers.assessment_request(configs.AGENT1_ID, 0, oa_zones, oa_hits)
            print(_msg)
            self.controlpub.publish(_msg)
        except Exception as e:
            print(e)
    ...
    def robot2_assessment_button_click(self):
        try:
            self.robot2_assessment_button.setStyleSheet("background-color: green")
            oa_zones = 5
            oa_hits = 5
            _msg = configs.MessageHelpers.assessment_request(configs.AGENT2_ID, 0, oa_zones, oa_hits)
            print(_msg)
            self.controlpub.publish(_msg)
        except Exception as e:
            print(e)
    ...
    def clear_robot1_assessment_button(self):
        self.robot1_assessment_button.setStyleSheet("")

    def clear_robot2_assessment_button(self):
        self.robot2_assessment_button.setStyleSheet("")

    def agent1_assessment_update(self, msg):
        """ Update the assessment display for agent 1 """
        try:
            self.clear_robot1_assessment_button()
    ...
    def agent2_assessment_update(self, msg):
        ...
        try:
            self.clear_robot2_assessment_button()
            target_goal = msg['target']
            collisions = msg['collisions']
            zones = msg['zones']
            predicted_craters = msg['predicted_craters']
